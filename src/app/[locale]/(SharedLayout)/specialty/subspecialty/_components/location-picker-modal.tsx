...
export default function LocationPickerModal({ isOpen, onClose, onConfirmLocation }: LocationPickerModalProps) {
  ...
  useEffect(() => {
    ...
    } else if (isOpen && window.google && mapRef.current && !showSavedLocations) {
      initializeMap()
    }
              ...
              {savedLocations.map((location) => (
                <div
                  ...
                  className={`flex flex-col sm:flex-row justify-between items-center p-4 rounded-2xl transition-all ${
                    selectedSavedLocation === location.id ? "bg-[#eff6fe]" : "border border-[#d0d5dd] hover:border-[#62a0f6]"
                  }`}
                >
                  {/* Location Info */}
